...
export default function Layout({
  ...
  // // Run after every 5s
  // useEffect(() => {
  //   let timerID = setInterval(() => getTime(), 500000);
  ...
  // const timeExpired = formatDate(getCookieUserLogin()?.expiredAt).props.children.props.children[2].props.children[1].split(':')
  // var timeExpiredToSecond = (+timeExpired[0]) * 60 * 60 + (+timeExpired[1]) * 60 + (+timeExpired[2]);

  // useEffect(() => {
  //   // Get time local & convert to second
  //   const a = time.toLocaleTimeString().split(':')
  //   var seconds = (+a[0]) * 60 * 60 + (+a[1]) * 60 + (+a[2]); 
  //   // Check if time local + 3000 > expired time => call refresh token

  //     API.post(endpoints.refreshToken, {remember_me: false} ,{ headers: headers.headers_token, withCredentials: true })
  //     .then(res => {
  //       console.log({res})
  //       setCookieUserLogin(res.data.data.token)
